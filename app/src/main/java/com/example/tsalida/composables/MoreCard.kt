...
fun MoreCard(painter: Painter = painterResource(R.drawable.megaphone), title: String = "Responsive\n Reading", navController: NavHostController, route: String){
    Surface(color = Color.White, shape = RoundedCornerShape(10.dp), modifier = Modifier.height(250.dp).width(150.dp).clickable{navController.navigate(route)}, shadowElevation = 2.dp) {
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            Spacer(Modifier.height(10.dp))
            Image(painter, "$title Icon", Modifier.padding(20.dp, 30.dp).size(100.dp))
            Spacer(Modifier.height(20.dp))
            Text(title,  textAlign = TextAlign.Center, fontWeight = FontWeight.SemiBold, fontSize = 20.sp)
